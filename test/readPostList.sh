...
if [ $1 ] && [ $2 ]
then
query="query readPostList{readPostList(n:$1,offset:$2){totalCount offset result{_id author title content date}}}"
else 
#default 5 0
query="query readPostList{readPostList(n:5,offset:0){totalCount offset result{_id author title content date}}}"
fi
